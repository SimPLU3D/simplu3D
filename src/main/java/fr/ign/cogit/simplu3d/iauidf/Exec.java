...
//       if (imu != 78020440) continue;
  ...
  public static boolean simulRegulationByIMU(int imu, List<Regulation> lReg, String folderImu) throws Exception {
    ...
    for (int i = 0; i < nbBPU; i++) {
      ...
      IFeatureCollection<IFeature> featCTemp = simulRegulationByBasicPropertyUnit(env, env.getBpU().get(i), imu, lReg);
   ...
   * @throws Exception
   */
  public static IFeatureCollection<IFeature> simulRegulationByBasicPropertyUnit(Environnement env, BasicPropertyUnit bPU, int imu, List<Regulation> lReg)
      throws Exception {
    // Stocke les résultats en sorties
    IFeatureCollection<IFeature> featC = new FT_FeatureCollection<>();
