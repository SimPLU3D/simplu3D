...
public class MultipleBuildingsCuboid {
	...
	public static Sampler<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> create_sampler(
		...
		IGeometry geomBand1 = r1.getGeomBande().intersection(
				bP.getLineRoad().buffer(d[3] / 2 + v[3]));
		IGeometry geomBand2 = null;
		if (r2 != null) {
			geomBand2 = r2.getGeomBande();
		}

		Transform transformBand1 = new ParallelPolygonTransform(d2, v,
				geomBand1, bP.getLineRoad().toArray());
		...
		double p_simple = 0.5;
		ObjectBuilder<Cuboid> builderBand1 = new ParallelBuilder(bP
				.getLineRoad().toArray(), 1);
		...
		if (r2 != null && r2.getArt_71() == 2) {
			...
			IFeatureCollection<SpecificCadastralBoundary> featC = bpU
					.getCadastralParcel()
					.get(0)
					.getSpecificSideBoundary(
			...
			builderBand2 = new ObjectBuilder<Cuboid>() {
				...
				public Cuboid build(double[] coordinates) {
					return new SimpleCuboid(coordinates[0], coordinates[1],
		...
		if (geomBand1 != null && !geomBand1.isEmpty()) {
			...
			lKernelsBand1 = getBande1Kernels(variate, nullView, p,
					transformBand1, builderBand1);
		...
		if (geomBand2 != null && !geomBand2.isEmpty()) {
			...
			lKernelsBand2 = getBande2Kernels(variate, nullView, p,
					transformBand2, builderBand2, band2parallel);
		...
		CuboidSampler2 objectSampler = new CuboidSampler2(rng, p_simple,
				transformBand1, transformBand2, builderBand1, builderBand2);
	...
	private static List<Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>>> getBande1Kernels(
			Variate variate,
			NullView<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> nullView,
			Parameters p, Transform transformParallel,
		...
		if ((p.getDouble("maxheight") - p.getDouble("minheight")) > 0.2) {
			...
			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> parallelHeight = new Kernel<>(
					pView, pView, variate, variate, new ChangeValue(
							amplitudeHeight, 5, 3), 0.2, 1.0, "ChgHeightP");
			kernels.add(parallelHeight);
		}

		Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> parallelMovekernel = new Kernel<>(
				pView, pView, variate, variate, new MoveParallelCuboid(
						p.getDouble("amplitudeMove")), 0.2, 1.0, "SimpleMoveP");
		kernels.add(parallelMovekernel);

		Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> parallelLength = new Kernel<>(
				pView, pView, variate, variate, new ChangeValue(
						p.getDouble("amplitudeMaxDim"), 5, 2), 0.2, 1.0,
				"ChgLengthP");
	...
	private static List<Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>>> getBande2Kernels(
			Variate variate,
			NullView<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> nullView,
			Parameters p, Transform transformSimple,
		...
		if (parallel) {
			...
			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> kernel2 = new Kernel<>(
					nullView, pView, variate, variate, transformSimple,
			...
			if ((p.getDouble("maxheight") - p.getDouble("minheight")) > 0.2) {
				...
				Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> parallelHeight = new Kernel<>(
						pView, pView, variate, variate, new ChangeValue(
								amplitudeHeight, 5, 3), 0.2, 1.0, "ChgHeightP2");
				kernels.add(parallelHeight);
			}

			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> parallelMovekernel = new Kernel<>(
					pView, pView, variate, variate, new MoveParallelCuboid(
							p.getDouble("amplitudeMove")), 0.2, 1.0,
					"SimpleMoveP2");
			kernels.add(parallelMovekernel);
			...
			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> simpleMovekernel = new Kernel<>(
					sView, sView, variate, variate, new MoveCuboid(
							p.getDouble("amplitudeMove")), 0.2, 1.0,
					"SimpleMove");
			...
			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> simpleRotatekernel = new Kernel<>(
					sView, sView, variate, variate, new RotateCuboid(
							p.getDouble("amplitudeRotate") * Math.PI / 180),
					0.2, 1.0, "RotateS");
			...
			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> simpleWidthkernel = new Kernel<>(
					sView, sView, variate, variate, new ChangeWidth(
							p.getDouble("amplitudeMaxDim")), 0.2, 1.0,
					"ChgWidthS");
			kernels.add(simpleWidthkernel);

			Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> simpleLength = new Kernel<>(
					sView, sView, variate, variate, new ChangeLength(
							p.getDouble("amplitudeMaxDim")), 0.2, 1.0,
					"ChgLengthS");
			...
			if ((p.getDouble("maxheight") - p.getDouble("minheight")) > 0.2) {
				...
				Kernel<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> simpleHeight = new Kernel<>(
						sView, sView, variate, variate, new ChangeHeight(
	...
	public static class ParallelBuilder implements ObjectBuilder<Cuboid> {
		...
		public ParallelBuilder(IGeometry[] limits, int bandType)
				throws Exception {
		...
		public Cuboid build(double[] coordinates) {
			...
			if(bandType == 1){
				...
				result 	= new ParallelCuboid(coordinates[0],
						coordinates[1], coordinates[2], distance * 2,
						coordinates[3], orientation + Math.PI / 2);
				
			}else{
				result 	= new ParallelCuboid2(coordinates[0],
						coordinates[1], coordinates[2], distance * 2,
						coordinates[3], orientation + Math.PI / 2);
	...
	public static class CuboidSampler2 implements ObjectSampler<Cuboid> {
		...
		public CuboidSampler2(RandomGenerator e, double p_simple,
	 ...
	 * this.transformParallel = transformParallel; this.variate = new
	 * Variate(e); }
	 * 
	 * @Override public double sample(RandomGenerator e) { double[] val0;
	 * double[] val1; if (engine.nextDouble() < p_simple) { val0 = new
	 * double[6]; val1 = new double[6]; double phi = this.variate.compute(val0,
	 * 0); double jacob = this.transformSimple.apply(true, val0, val1);
	 * this.object = sbuilder.build(val1); return phi / jacob; } val0 = new
	 * double[4]; val1 = new double[4]; double phi = this.variate.compute(val0,
	 * 0); double jacob = this.transformParallel.apply(true, val0, val1);
	 * this.object = pbuilder.build(val1); return phi / jacob; }
	 * 
	 * @Override public double pdf(Cuboid t) { if
	 * (SimpleCuboid.class.isInstance(t)) { double[] val1 = new double[6];
	 * sbuilder.setCoordinates(t, val1); double[] val0 = new double[6]; double
	 * J10 = this.transformSimple.apply(false, val1, val0); double pdf =
	 * this.variate.pdf(val0, 0); return pdf * J10; } double[] val1 = new
	 * double[4]; pbuilder.setCoordinates(t, val1); double[] val0 = new
	 * double[4]; double J10 = this.transformParallel.apply(false, val1, val0);
	 * if (J10 == 0) { return 0; } double pdf = this.variate.pdf(val0, 0);
	 * return pdf * J10; }
	 * 
