...
public class OptimisedRCuboidDirectRejection extends DefaultSimPLU3DOptimizer<CuboidRoofed> {
	public GraphConfiguration<CuboidRoofed> process(RandomGenerator rG, BasicPropertyUnit bpu, SimpluParameters p,
		...
		Sampler<GraphConfiguration<CuboidRoofed>, BirthDeathModification<CuboidRoofed>> samp = create_sampler(rG, p,
				bpu, pred, polygon);
		// Température
	...
	public GraphConfiguration<CuboidRoofed> create_configuration(SimpluParameters p, IGeometry geom,
	...
	public GraphConfiguration<CuboidRoofed> create_configuration(SimpluParameters p, Geometry geom,
		...
		UnaryEnergy<CuboidRoofed> energyVolumePondere = new MultipliesUnaryEnergy<CuboidRoofed>(ponderationVolume,
				energyVolume);

		// On retire de l'énergie de création, l'énergie de l'aire
		UnaryEnergy<CuboidRoofed> u3 = new MinusUnaryEnergy<CuboidRoofed>(energyCreation, energyVolumePondere);
		...
		UnaryEnergy<CuboidRoofed> u4 = new DifferenceVolumeUnaryEnergy<CuboidRoofed>(geom);
		UnaryEnergy<CuboidRoofed> u5 = new MultipliesUnaryEnergy<CuboidRoofed>(ponderationDifference, u4);
		UnaryEnergy<CuboidRoofed> unaryEnergy = new PlusUnaryEnergy<CuboidRoofed>(u3, u5);
		...
		GraphConfiguration<CuboidRoofed> conf = new GraphConfiguration<>(unaryEnergy, binaryEnergy);
		return conf;
	}
	 ...
	 * @param p       the parameters from a json file
	 * @param bpU     a basic property unit for the simulation
	 * @param pred    a predicate that will check if the rules are respected
	 * @param polygon a polygon that will contain all the cuboids
	 * @return a sampler for the optimization
	 */
	public Sampler<GraphConfiguration<CuboidRoofed>, BirthDeathModification<CuboidRoofed>> create_sampler(
		...
		UniformBirth<CuboidRoofed> birth = new UniformBirth<CuboidRoofed>(rng,
				new CuboidRoofed(xmin, ymin, lmin, wmin, hgmin, orientationMin, htmin, deltaMin),
				new CuboidRoofed(xmax, ymax, lmax, wmax, hgmax, orientationMax, htmax, deltaMax), builder,
				TransformToSurface.class, /* (IGeometry) */ polygon);
		...
		kernels.add(
				factory.make_uniform_modification_kernel(rng, builder, new MoveRCuboid(amplitudeMove), 0.2, "Move"));
