...
public class OptimisedBuildingsCuboidFinalDirectRejectionParallelTempering  extends DefaultSimPLU3DOptimizer<Cuboid> {
  ...
  public void process(BasicPropertyUnit bpu, Parameters p, Environnement env, int id,
    ...
    Sampler<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> sampler = create_sampler(Random.random(), p, bpu, pred);
    ...
    for (int i = 0; i < this.numberOfReplicas; i++) {
      ...
      tabConfig[i] = conf;
      samp[i] = new GeometricSchedule<SimpleTemperature>(new SimpleTemperature(i * Math.log(this.tempMax) / (this.numberOfReplicas - 1)),
          1);
   ...
   *         prise en compte
   */
  public static GraphConfiguration<Cuboid> create_configuration(Parameters p, Geometry geom, BasicPropertyUnit bpu) {
   ...
   * @return
   */
  Sampler<GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> create_sampler(RandomGenerator rng, Parameters p,
