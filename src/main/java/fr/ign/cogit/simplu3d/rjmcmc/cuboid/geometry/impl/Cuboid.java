...
public class Cuboid extends AbstractSimpleBuilding implements Primitive {
	...
	public Polygon toGeometry() {
		if (geomJTS == null) {
			...
			/*
			 * double hLength = length / 2; double hWidth = width / 2; pts[0] =
			 * new Coordinate(this.centerx - hLength, this.centery - hWidth);
			 * pts[1] = new Coordinate(this.centerx + hLength, this.centery -
			 * hWidth, height); pts[2] = new Coordinate(this.centerx + hLength,
			 * this.centery + hWidth); pts[3] = new Coordinate(this.centerx -
			 * hLength, this.centery + hWidth, height); pts[4] = new
			 * Coordinate(pts[0]);
			 */
	...
	public String toString() {
		return "Cuboid : " + " Centre " + this.centerx + "; " + this.centery + " hauteur " + this.height + " largeur " + this.width
	...
	public Rectangle2D getRectangle2D() {
		...
		if (rectangle == null) {
			rectangle = new Rectangle2D(this.centerx, this.centery, Math.cos(orientation) * length / 2, Math.sin(orientation) * length / 2,
					width / length);
		}
		return rectangle;
	}
	...
	public static boolean do_intersect(Cuboid a, Cuboid b) {
		return IntersectionArea.do_intersect(a.getRectangle2D(), b.getRectangle2D());
	}

	public static double intersection_area(Cuboid a, Cuboid b) {
		return IntersectionArea.intersection_area(a.getRectangle2D(), b.getRectangle2D());
	}
