...
public class Cuboid extends AbstractSimpleBuilding {
  ...
  public double intersectionArea(Primitive p) {
    ...
    if (p instanceof Cuboid) {
      return this.getRectangle2D()
          .intersectionArea(((Cuboid) p).getRectangle2D());
    }
  ...
  public List<AbstractBuilding> bandEpsilon(Geometry geom, double distMin,
    ...
    Geometry geomOut = Double.isNaN(distMax) ? geomJTS
        : geom.buffer(distMax).intersection(geomJTS);
    ...
    try {
      lS = FromGeomToSurface
          .convertGeom(JtsGeOxygene.makeGeOxygeneGeom(geomOut));
      for (IOrientableSurface oS : lS) {
        ...
        lDMax.add(distMax);
        lPolygonOut.add(
            new FastBuildingPart((IPolygon) oS, this.height + this.getZmin()));
        lFP.add(fBP);
      }
    } catch (Exception e) {
      // TODO Auto-generated catch block
  ...
  public boolean prospect(IGeometry geom, double slope, double hIni) {
    try {
      return prospectJTS(AdapterFactory.toGeometry(geomFact, geom), slope,
          hIni);
    } catch (Exception e) {
  ...
  public IGeometry generated3DGeom() {
    ...
    List<IDirectPositionList> lDpl = createCubePointsList();
  ...
  public List<IDirectPositionList> createCubePointsList() {
    if (pointsCube == null) {
      double zmin = getZmin();
