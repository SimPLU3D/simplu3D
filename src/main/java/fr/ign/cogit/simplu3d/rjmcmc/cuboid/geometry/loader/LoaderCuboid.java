...
public class LoaderCuboid {
  ...
  public static List<Cuboid> loadFromCollection(IFeatureCollection<IFeature> featColl) {
  ...
  public static Cuboid transformFeature(IFeature feat) {
    ...
    Vecteur v = new Vecteur(
        (dpl.get(1).getX() + dpl.get(2).getX()) / 2 - centreX,
        (dpl.get(1).getY() + dpl.get(2).getY()) / 2 - centreY);
    Angle a = v.direction();
    return new Cuboid(centreX, centreY, l, w, oBB.getzMax() - oBB.getzMin(), a.getValeur());
  }
}
