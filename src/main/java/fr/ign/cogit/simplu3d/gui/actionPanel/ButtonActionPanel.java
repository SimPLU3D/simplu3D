package fr.ign.cogit.simplu3d.gui.actionPanel;
 ...
 * @version 1.0
 *
 *          Classe permettant d'appliquer des modifications sur les bâtiments
 *          paramétrés
 * 
 *
 */
public class ButtonActionPanel extends JPanel implements ActionListener {
	...
	public ButtonActionPanel(InterfaceMap3D iMap3D) {
		...
		this.setLayout(null);
		this.setBorder(new TitledBorder(new EtchedBorder(), Messages.getString("Action à porter sur le bâtiment")));
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (ParametricBuilding bP : bPList) {
			...
			RepresentationBatiment pBIni = (RepresentationBatiment) bP.getRepresentation();

			bP.setRepresentation(
					new RepresentationBatiment(bP, pBIni.isRepresentFaitage(), pBIni.isRepresentGouttiere()));
