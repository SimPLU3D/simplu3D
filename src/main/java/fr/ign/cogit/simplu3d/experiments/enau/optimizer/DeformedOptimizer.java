...
public class DeformedOptimizer  extends DefaultSimPLU3DOptimizer<Cuboid> {
  ...
  public GraphConfiguration<DeformedCuboid> process(BasicPropertyUnit bpu, Parameters p, Environnement env, int id,
      ...
      double distReculVoirie, double slope, double hIni, double hMax, double distReculLimi, double slopeProspect, double maximalCES,
    ...
    try {
      conf = create_configuration(p, geom, bpu, distReculVoirie, slope, hIni, hMax, distReculLimi, slopeProspect, maximalCES);
    } catch (Exception e) {
      e.printStackTrace();
    }
    // Création de l'échantilloneur
    Sampler<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> samp = create_sampler(Random.random(), p, bpu, pred,
        pos);
    // Température
    ...
    if (p.getBoolean("outputstreamvisitor")) {
      Visitor<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> visitor = new OutputStreamVisitor<>(System.out);
      list.add(visitor);
    }
    if (p.getBoolean("shapefilewriter")) {
      ShapefileVisitor<DeformedCuboid, GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> shpVisitor = new ShapefileVisitor<>(
    ...
    list.add(countV);
    CompositeVisitor<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> mVisitor = new CompositeVisitor<>(list);
  ...
  static void init_visitor(Parameters p, Visitor<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> v) {
  ...
  public GraphConfiguration<DeformedCuboid> create_configuration(Parameters p, IGeometry geom, BasicPropertyUnit bpu,
    ...
    return this.create_configuration(p, AdapterFactory.toGeometry(new GeometryFactory(), geom), bpu, distReculVoirie, slope, hIni, hMax,
        distReculLimi, slopeProspect, maximalCES);
   ...
   *         prise en compte
   */
  public GraphConfiguration<DeformedCuboid> create_configuration(Parameters p, Geometry geom, BasicPropertyUnit bpu, double distReculVoirie,
      double slope, double hIni, double hMax, double distReculLimi, double slopeProspect, double maximalCES) {
    // Énergie constante : à la création d'un nouvel objet

    double energyCrea = Double.isNaN(this.energyCreation) ? p.getDouble("energy") : this.energyCreation;

    ConstantEnergy<DeformedCuboid, DeformedCuboid> energyCreation = new ConstantEnergy<DeformedCuboid, DeformedCuboid>(energyCrea);
    ...
    UnaryEnergy<DeformedCuboid> energyVolumePondere = new MultipliesUnaryEnergy<DeformedCuboid>(ponderationVolume, energyVolume);
    ...
    BinaryEnergy<DeformedCuboid, DeformedCuboid> binaryEnergy = new MultipliesBinaryEnergy<DeformedCuboid, DeformedCuboid>(c3, b1);
   ...
   * @throws Exception
   */
  public Sampler<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> create_sampler(RandomGenerator rng,
      Parameters p, BasicPropertyUnit bpU,
      ConfigurationModificationPredicate<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> pred, int pos)
          throws Exception {
  ...
  private static IMultiCurve<IOrientableCurve> shiftRoad(BasicPropertyUnit bPU, double valShiftB, IMultiCurve<IOrientableCurve> iMS) {
  ...
  public static class ParallelBuilder implements ObjectBuilder<DeformedCuboid> {
    ...
    public DeformedCuboid build(double[] coordinates) {
      ...
      ParallelDeformedCuboid result = new ParallelDeformedCuboid(coordinates[0], coordinates[1], coordinates[2], distance * 2,
          coordinates[4], coordinates[5], coordinates[6], coordinates[7], orientation + Math.PI / 2);
  ...
  public Sampler<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> determinedPos(RandomGenerator rng,
      Parameters p, BasicPropertyUnit bpU,
      ConfigurationModificationPredicate<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> pred,
    ...
    double[] d = new double[] { env.maxX(), env.maxY(), maxlen, maxwid, maxheight, maxheight, maxheight, maxheight, Math.PI };
    double[] v = new double[] { env.minX(), env.minY(), minlen, minwid, minheight, minheight, minheight, minheight, 0. };

    for (int i = 0; i < d.length; i++) {
      d[i] = d[i] - v[i];
    }

    ParallelDeformedCuboidTransform transformParallel = new ParallelDeformedCuboidTransform(d, v, bpU.getpol2D(), ims.toArray());
    ...
    List<Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>>> kernels = new ArrayList<>(3);
    ...
    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> kernel = new Kernel<>(nullView, pView, variate,
        variate, transformParallel, p.getDouble("pbirth"), p.getDouble("pdeath"), "Parallel");

    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelMovekernel = new Kernel<>(pView, pView,
        variate, variate, new MoveParallelDeformedCuboid(p.getDouble("amplitudeMove")), 0.2, 1.0, "SimpleMoveP");
    kernels.add(parallelMovekernel);

    double amplitudeDim = p.getDouble("amplitudeMaxDim");

    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelHeight = new Kernel<>(pView, pView, variate,
        variate, new ChangeValue(amplitudeDim, 10, 3), 0.2, 1.0, "ChangeLength");
    kernels.add(parallelHeight);

    double amplitudeHeight = p.getDouble("amplitudeHeight");
    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelHeight1 = new Kernel<>(pView, pView, variate,
        variate, new ChangeHeight1Deformed(amplitudeHeight), 0.2, 1.0, "ChangeHeight1");
    kernels.add(parallelHeight1);

    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelHeight2 = new Kernel<>(pView, pView, variate,
        variate, new ChangeHeight2Deformed(amplitudeHeight), 0.2, 1.0, "ChangeHeight2");
    kernels.add(parallelHeight2);

    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelHeight3 = new Kernel<>(pView, pView, variate,
        variate, new ChangeHeight3Deformed(amplitudeHeight), 0.2, 1.0, "ChangeHeight3");
    kernels.add(parallelHeight3);

    Kernel<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> parallelHeight4 = new Kernel<>(pView, pView, variate,
        variate, new ChangeHeight4Deformed(amplitudeHeight), 0.2, 1.0, "ChangeHeight4");
    kernels.add(parallelHeight4);

    kernels.add(kernel);

    Sampler<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> s = new GreenSamplerBlockTemperature<>(rng, ds,
        new MetropolisAcceptance<SimpleTemperature>(), kernels);
    return s;
  }

  public Sampler<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> undeterminedSampler(RandomGenerator rng,
      Parameters p, BasicPropertyUnit bpU,
      ConfigurationModificationPredicate<GraphConfiguration<DeformedCuboid>, BirthDeathModification<DeformedCuboid>> pred) {
    ...
    UniformBirth<DeformedCuboid> birth = new UniformBirth<DeformedCuboid>(
        rng, new DeformedCuboid(env.minX(), env.minY(), minlen, minlen, minheight, minheight, minheight, minheight, 0),
        new DeformedCuboid(env.maxX(), env.maxY(), maxlen, maxlen, maxheight, maxheight, maxheight, maxheight, Math.PI), builder,
    ...
    kernels.add(factory.make_uniform_birth_death_kernel(rng, builder, birth, p.getDouble("pbirth"), 0.5, "BirthDeath"));

    double amplitudeMove = p.getDouble("amplitudeMove");
    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new MoveCuboidDeformed(amplitudeMove), 0.2, "Move"));

    double amplitudeMaxDim = p.getDouble("amplitudeMaxDim");
    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeValueDeformed(amplitudeMaxDim, 10, 2), 0.2, "ChgWidth"));

    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeValueDeformed(amplitudeMaxDim, 10, 3), 0.2, "ChgLength"));

    double amplitudeHeight = p.getDouble("amplitudeHeight");

    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeHeight1Deformed(amplitudeHeight), 0.2, "ChgHeight1"));

    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeHeight2Deformed(amplitudeHeight), 0.2, "ChgHeight2"));

    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeHeight3Deformed(amplitudeHeight), 0.2, "ChgHeight3"));

    kernels.add(factory.make_uniform_modification_kernel(rng, builder, new ChangeHeight4Deformed(amplitudeHeight), 0.2, "ChgHeight4"));
  ...
  private EndTest create_end_test_stability(Parameters p) {
    ...
    return new StabilityEndTest<DeformedCuboid>(p.getInteger("nbiter"), loc_deltaconf);
  }
  ...
  public static class ParalledDeformedSampler implements ObjectSampler<DeformedCuboid> {
    ...
    public ParalledDeformedSampler(RandomGenerator e, Transform transformBand1, ObjectBuilder<? extends DeformedCuboid> builder) {
      this.engine = e;
