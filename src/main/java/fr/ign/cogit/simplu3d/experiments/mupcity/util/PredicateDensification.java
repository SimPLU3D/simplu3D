...
public class PredicateDensification<O extends Cuboid, C extends AbstractGraphConfiguration<O, C, M>, M extends AbstractBirthDeathModification<O, C, M>> implements ConfigurationModificationPredicate<C, M> {
	...
	public PredicateDensification(BasicPropertyUnit bPU, Regulation r1, Regulation r2) throws Exception {
		super();
		this.currentBPU = bPU;
		this.r1 = r1;
		this.r2 = r2;
	...
	/**
	 * Initialisation : stocke les limites séparatives dans les géométries jts jtsCurveLimiteSepParcel et jtsCurveLimiteFrontParcel.
	 * 
	 * @throws Exception
	 */
			...
			for (ParcelBoundary sCB : cP.getBoundaries()) {
				...
				if (sCB.getType() == ParcelBoundaryType.LAT) {
					...
					if (geom instanceof IOrientableCurve && !geom.isEmpty()) {
						...
						if (r1 != null && r1.getArt_71() == 2 && (sCB.getSide() != PredicateDensification.RIGHT_OF_LEFT_FOR_ART_71) || r2 != null && r2.getArt_71() == 2 && (sCB.getSide() != PredicateDensification.RIGHT_OF_LEFT_FOR_ART_71)) {
	...
	public boolean check(C c, M m) {
		...
		if (birth != null) {
			...
			if (birth instanceof ParallelCuboid) {
				...
				System.out.println("Predicate IAUIDF - Unexpected class during object birth : " + birth.getClass().getCanonicalName());
			}
	...
	private boolean checkDistanceInterBuildings(C c, M m, double distanceInterBati) {
		...
		while (iTBat.hasNext()) {
			...
			for (O ab : lO) {
				...
				if ((new SquaredDistance(ab.getRectangle2D(), batTemp.getRectangle2D())).getSquaredDistance() < distComp * distComp) {
					return false;
				}
