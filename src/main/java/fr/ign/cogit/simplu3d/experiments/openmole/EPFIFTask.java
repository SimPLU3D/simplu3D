...
public class EPFIFTask {
	...
	public static String run(File folder, String dirName, File folderOut, File parameterFile, long seed)
		...
		Map<String, List<Regulation>> regulation = loadRules(new File(folder + "/parcelle.shp"), Integer.parseInt(imu));
		...
		for (BasicPropertyUnit bPU : env.getBpU()) {
			...
			if (lR != null && !lR.isEmpty()) {
				...
				IFeatureCollection<IFeature> feats = simulationForEachBPU(env, bPU, lR, Integer.parseInt(imu),
						parameterFile);
				if (!feats.isEmpty()) {
					featC.addAll(feats);
